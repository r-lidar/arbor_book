...
## Common mistakes {#sec-oversegmentation}
...
Arbor `v1.1.0` introduced an over-segmentation solver module. Starting in `v1.1.0`, `segment_instance()` automatically detects and fixes a significant number of over-segmentation cases. The module is enabled by default and can be turned off (restoring `v1.0.0` behavior) via:
...
![Arbor v1.0.0 tends to oversegment some trees, and especially tends to oversegment large buttress trees. v1.1.0 introduces an over-segmentation solver that significantly reduces the occurrence of such problems (which were already relatively rare).](figures/chap_instance/overfitting.png)

...
ArborStudio also integrates the latest developments from the core library, including features not yet publicly available. In addition, it includes memory optimizations that are not possible within the R package, making it significantly more efficient for large datasets.

**ArborStudio is currently under active development** and is not yet publicly available. It may or may not be released as open source. While the Arbor license (GPL-3) prohibits third parties from building closed-source software on top of Arbor, we are the copyright holders and therefore retain the right to distribute ArborStudio under a different licensing model.

Depending on the level of community interest and financial support for ArborStudio’s development, the software may eventually be released either for free or under a commercial license.
